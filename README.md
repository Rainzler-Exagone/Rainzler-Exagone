...
**Backend**
`Node.js` `Express` `MySQL` `MongoDB` `PostgreSQL` `Java` `C` `Python` `.NET`

**Mobile & Desktop**
`Flutter` `Dart` `Electron`

**Tools & Workflow**
`Git` `Docker` `Figma` `Postman` `GCP` `Bash` `Jest` `Linux`
...
<!--
🐍 Optional: a live "contribution snake" animation that eats your commit graph.
It needs a one-time GitHub Action in this repo, then you embed the generated SVG:
...
jobs:
  generate:
    ...
    steps:
      - uses: Platane/snk@v3
        with:
          github_user_name: Rainzler-Exagone
